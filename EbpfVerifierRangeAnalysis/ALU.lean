...
theorem zero_lt_UInt32_max : (0 : UInt32) < UInt32.max := by
  -- UInt32.max is (2^32 - 1), which is definitely greater than 0
  -- So convert to Nat and compare:
  have h := UInt32.lt_iff_toNat_lt 0 UInt32.max
  rw [UInt32.zero] at h
  rw [UInt32.max] at h
  -- Now show 0 < (2^32 - 1) as Nat:
...
theorem scalar_add_32_range_bounds
  ...
  by
    let c := scalar_add_32 a b
    -- unfold scalar_add_32 to get the overflow check and result bounds
